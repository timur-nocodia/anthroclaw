...
export const RUNTIME_FEATURE_CONTRACTS: RuntimeFeatureContract[] = [
  ...
  feature(
    'tools.side-effect-gates',
    'tools_permissions',
    'Provider-neutral side-effect gate contract',
    ['src/runtime/side-effect-gate.ts', 'docs/runtime-migration-rules.md'],
    'Live side-effect evidence must be expressed as reusable runtime gates whose agent identity, route, tool/action, policy assertions, expected effects, cleanup checks, and metrics are inputs.',
    'New Pi/runtime migration work cannot introduce agent-specific runtime primitives; concrete agents appear only as fixtures or expansion-packet evidence over the generic gate contract.',
    ['src/runtime/__tests__/side-effect-gate.test.ts', 'docs/runtime-migration-rules.md'],
    'Runtime replacement must prove real side effects without turning individual lab agents into framework APIs.',
  ),
  feature(

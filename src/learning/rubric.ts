...
export function buildRubricLearningReviewPrompt(input: BuildRubricLearningReviewPromptInput): string {
  ...
  const prompt = [
    ...
    'Return ONLY strict JSON with this exact shape (no extra top-level keys, no extra keys inside each action):',
    '{"actions":[{"type": "...", "title"?: "...", "confidence"?: 0.0-1.0, "rationale"?: "...", "rubric"?: { ... }, "payload": { ... }}]}',
    '',
    'The "rubric" field is OPTIONAL and, when present, MUST be a nested object with exactly these keys:',
    ...
    'Do NOT place these rubric keys at the top level of an action. They live ONLY inside "rubric".',
    '',
    'Concrete example of a single well-formed action:',
    '{"type":"memory_candidate","title":"Remember user preference","rubric":{"evidenceStrength":"strong","durability":"durable","reusability":"agent_specific","safety":"safe","recommendedActionClass":"memory"},"payload":{"text":"User prefers short summaries.","kind":"preference","reason":"explicit correction"}}',
    '',
    'Allowed actions:',

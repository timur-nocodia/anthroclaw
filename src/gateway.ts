...
export class Gateway {
  ...
  async handleCallbackQuery(cb: import('./channels/types.js').CallbackEvent): Promise<void> {
    ...
    if (this.handleApprovalCallback(cb.data)) {
      await ack();
      return;
    }
   ...
   * Returns true if the payload matched "approve:<id>" or "deny:<id>" and was
   * forwarded to the ApprovalBroker; false if the payload is unrecognised.
   */

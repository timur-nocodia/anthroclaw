...
import { DecisionCenter } from './decisions/center.js';
...
export class Gateway {
  ...
  private decisionCenter: DecisionCenter | null = null;
  /**
  ...
  async handleCallbackQuery(cb: import('./channels/types.js').CallbackEvent): Promise<void> {
    ...
    const decisionResult = await this.handleDecisionCallback(cb);
    if (decisionResult.handled) {
      await ack(decisionResult.message);
      return;
    }

    // Approval/deny buttons generated by promptForApproval() — resolve broker first
    if (this.handleApprovalCallback(cb.data, cb.senderId)) {
      await ack();
      return;
    }
  ...
  private async handleDecisionCallback(cb: import('./channels/types.js').CallbackEvent): Promise<{ handled: boolean; message?: string }> {
    const parsed = parseDecisionCallbackData(cb.data);
    if (!parsed) return { handled: false };
    const result = this.decisionCenter?.resolveEvent({
    ...
    }) ?? { handled: false, reason: 'not_ready' };
    const finalized = await this.finalizeDecisionResolution(result.decision);
    return {
      handled: true,
      message: formatDecisionResolutionMessage(finalized?.status ?? result.status, result.reason),
    };
  }

  /**
   * Handle an approval/deny callback_query payload.
   * Returns true if the payload matched "approve:<id>" or "deny:<id>" AND the
   * senderId matches the expected sender stored in the ApprovalBroker.
   * Returns false if the payload is unrecognised OR the sender is mismatched
   * (mismatch leaves the pending request in place).
   */
  handleApprovalCallback(data: string, senderId: string): boolean {
    const match = /^(approve|deny):(.+)$/.exec(data);
    if (!match) return false;
    const [, verb, id] = match;
    return this.approvalBroker.resolveBySender(id, senderId, verb === 'approve' ? 'allow' : 'deny');
  }
  ...
  private async handleDecisionTextReply(msg: InboundMessage): Promise<boolean> {
    ...
    if (channel) {
      await channel.sendText(msg.peerId, formatDecisionResolutionMessage(finalized?.status ?? result.status, result.reason), {
        accountId: msg.accountId,
        threadId: msg.threadId,
      }).catch((err) => {
        logger.warn({ err, decisionId: result.decision?.id, channel: msg.channel }, 'Decision text acknowledgement failed');
      });
    }
    return true;
  }

  private async finalizeDecisionResolution(decision?: DecisionRecord): Promise<DecisionRecord | undefined> {
    if (!decision || !this.learningStore) return decision;
    if (!decision.learningActionId) return decision;

    if (decision.kind === 'learning_memory' && decision.status === 'approved') {
      const action = this.learningStore.getAction(decision.learningActionId);
      const agent = this.agents.get(decision.agentId);
      if (!action || !agent) return decision;
      if (action.status === 'applied') {
        return this.decisionStore?.updateDecisionStatus(decision.id, 'applied', {
          reason: 'memory_already_applied',
        }) ?? decision;
      }
      if (action.status !== 'proposed' && action.status !== 'approved') return decision;
      ...
      try {
        applyMemoryCandidateAction({
          ...
          mode: agent.config.learning.mode,
          agentId: agent.id,
          channel: decision.originChannel,
          peerHash: hashIdentifier(decision.originPeerId),
          reviewStatusOverride: 'approved',
        });
        this.learningStore.updateActionStatus(action.id, 'applied', { appliedAt: Date.now() });
        return this.decisionStore?.updateDecisionStatus(decision.id, 'applied', {
          appliedAt: Date.now(),
        ...
        this.learningStore.updateActionStatus(action.id, 'failed', { error: message });
        return this.decisionStore?.updateDecisionStatus(decision.id, 'failed', {
          error: message,
  ...
  private enqueueLearningReviewAfterResponse(params: {
    ...
    const result = this.learningQueue.enqueueAfterResponse({
      ...
      metadata: {
        ...
        originChannel: params.msg.channel,
        originAccountId: params.msg.accountId,
        originPeerId: params.msg.peerId,
        originSenderId: params.msg.senderId,
        originThreadId: params.msg.threadId,
        originMessageId: params.msg.messageId,
...
function formatDecisionResolutionMessage(status?: string, reason?: string): string {
  if (reason === 'unauthorized') return 'Decision not authorized for this sender.';
  if (reason === 'ambiguous') return 'Multiple pending decisions. Use /learn approve CODE, /learn reject CODE, or /learn edit CODE.';
  if (reason === 'expired' || status === 'expired') return 'Decision expired.';
  if (reason === 'already_decided') return 'Decision already handled.';
  switch (status) {
    case 'approved':

...
describe('GET /api/security/profiles/[name]/baseline', () => {
  it('returns baseline for chat_like_anthroclaw', async () => {
    const { GET } = await import('@/app/api/security/profiles/[name]/baseline/route');
    const req = new NextRequest('http://localhost:3000/api/security/profiles/chat_like_anthroclaw/baseline');
    const res = await GET(req, { params: Promise.resolve({ name: 'chat_like_anthroclaw' }) });
  ...
  it('accepts legacy chat_like_openclaw as an alias', async () => {
    const { GET } = await import('@/app/api/security/profiles/[name]/baseline/route');

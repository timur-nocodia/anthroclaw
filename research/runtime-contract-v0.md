...
2. Run the manual **Pi smoke** GitHub Actions workflow, or run `pnpm smoke:pi-all -- --json` after the auth preflight passes, using the same optional storage path flags when staging should not read default Pi files.

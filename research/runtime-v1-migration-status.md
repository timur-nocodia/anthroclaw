...
- Ring 4 high-risk live automation has not started. This includes live production cron delivery, live proactive notifications, broad `send_message` fanout, and business-critical workflows.
...
| 7. Default-runtime rollout | In progress | Flip runtime default safely. | Default flip is merged into `main`; post-merge local, durable, live pull, safe Web UI, extended monitoring, Ring 1, Ring 2, and Ring 3 are green; Ring 4 high-risk automation remains. |
...
1. Define Ring 4 high-risk automation scope with an explicit rollback owner present.
2. Choose one live high-risk automation path only: cron delivery, proactive notification, broad `send_message`, or one business-critical workflow.
3. Run pre-Ring-4 `pnpm runtime:pi-monitor -- --since-minutes 60 --json --fail-on-alert`.
4. Execute the selected Ring 4 path with live stop-condition monitoring.

...
Continue the stacked migration from the smallest safe seam outward: with model loading, event text, session metadata, explicit tool policy, model-visible denial feedback, Pi event normalization, Claude-path Gateway `RuntimeEvent` consumption, and a non-default Pi Gateway headless bridge covered, the next useful proof is a real Pi Gateway run handle that streams normalized `RuntimeEvent` values and can expose production tool policy/interrupt semantics.

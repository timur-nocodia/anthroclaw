...
| `project-manager` | high by audit queue | Multi-root audit passed with coverage gap false and expansion packet recorded in `research/pi-expansion-packets/project-manager.md`. Current blockers are Telegram group route, `connect_mcp`, `manage_cron`, two external MCP servers, and required manual peer/thread plus tool-specific side-effect evidence. | Run automated external-MCP/plugin/monitor evidence and explicit operator go/no-go before any controlled group turn or scheduled-work side effect. |

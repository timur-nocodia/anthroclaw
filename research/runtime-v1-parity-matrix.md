...
Current multi-root expansion status is 62/67 evidence items closed (93%), with 0 missing packets. The remaining 5 evidence items are live/manual gates for `content_sm_building` and `project-manager`; `runtime:pi-expansion-status -- --open-only` prints that working queue directly while preserving the full summary metrics.
...
| `project-manager` | high by audit queue | Multi-root audit passed with coverage gap false and expansion packet recorded in `research/pi-expansion-packets/project-manager.md`. Automated pre-live evidence passed for monitor, external MCP, and plugin/context coverage; learning posture is propose-only by config audit. Current blockers are Telegram group route, `connect_mcp`, `manage_cron`, two external MCP servers, and required manual peer/thread plus tool-specific side-effect evidence. | Close manual peer/thread, tool-specific evidence, explicit operator go/no-go, controlled group/scheduled-work evidence, and post-expansion monitor before treating it as live-expanded. |

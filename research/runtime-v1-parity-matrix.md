...
| `tools.side-effect-gates` | tools_permissions | `src/runtime/__tests__/side-effect-gate.test.ts`; `src/runtime/__tests__/live-send-message-gate.test.ts`; `src/runtime/__tests__/live-send-media-gate.test.ts`; `src/runtime/__tests__/live-notification-gate.test.ts`; `runtime:pi-live-send-message-gate`; `runtime:pi-live-send-media-gate`; `runtime:pi-live-notification-gate`; `docs/runtime-side-effect-gates.md` | L1-L2 for message, media, and notification gates | Cron, Buildroom, external MCP, and config/admin side-effect gates still need the same generic extraction. |

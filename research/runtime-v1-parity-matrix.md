...
| `project-manager` | high by audit queue | Multi-root audit passed with coverage gap false and expansion packet recorded in `research/pi-expansion-packets/project-manager.md`. Automated pre-live evidence passed for monitor, external MCP, plugin/context coverage, generic Telegram group-route preflight, and propose-only learning posture; allowlisted peer/thread confirmation is closed by explicit account/peer/topic match. Current blockers are `connect_mcp`, `manage_cron`, two external MCP servers, and tool-specific side-effect evidence. | Close tool-specific evidence, explicit operator go/no-go, controlled group/scheduled-work evidence, and post-expansion monitor before treating it as live-expanded. |

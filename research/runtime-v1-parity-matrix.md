...
| Generic side-effect gate harness | Complete | Twelve provider-neutral gates cover controlled group live turns, messaging, media, notifications, cron notification, scheduled work, Buildroom handoff, admin/config, MCP/file-transfer, Honcho local, propose-only learning, and read-only memory; registry metadata, no-run plans, strict validation, dispatcher coverage, docs, and drift tests are in place. |
| Fleet expansion progress view | Complete | `runtime:pi-expansion-status` combines multi-root audit output with `research/pi-expansion-packets` and reports closed/open/missing packet state plus next actions in human or JSON form. It also classifies open evidence by kind and supports `--fail-on-open --allow-open-kind <kind>` plus `--allow-external-open` for CI/operator gates that should pass only when all remaining work is explicitly allowed; output includes the evaluated policy, exit code, reason, and concrete policy violations. |
...
| Tools and permissions | 6 | L4, with high-risk tools gated per agent | Built-in policy, approvals, file ownership, custom tools, external MCP proxy, generic side-effect gate contract, `send_message`, `send_media`, `manage_cron`, and escalation have canary/live or reusable gate evidence. Broad group fanout and customer paths stay expansion-gated. |
...
| `tools.side-effect-gates` | tools_permissions | `src/runtime/side-effect-gates/registry.ts`; `src/runtime/__tests__/side-effect-gate-registry.test.ts`; `src/runtime/__tests__/side-effect-gate.test.ts`; `src/runtime/__tests__/controlled-live-turn-gate.test.ts`; `src/runtime/__tests__/live-send-message-gate.test.ts`; `src/runtime/__tests__/live-send-media-gate.test.ts`; `src/runtime/__tests__/live-notification-gate.test.ts`; `src/runtime/__tests__/cron-notification-gate.test.ts`; `src/runtime/__tests__/buildroom-handoff-gate.test.ts`; `src/runtime/__tests__/admin-config-gate.test.ts`; `src/runtime/__tests__/mcp-file-transfer-gate.test.ts`; `src/runtime/__tests__/honcho-local-gate.test.ts`; `src/runtime/__tests__/learning-propose-gate.test.ts`; `src/runtime/__tests__/memory-read-gate.test.ts`; `src/cli/__tests__/pi-controlled-live-turn-gate.test.ts`; `src/cli/__tests__/pi-live-gate.test.ts`; `runtime:pi-live-gate -- --list --json`; `runtime:pi-live-gate -- --describe <gate-id> --json`; `runtime:pi-live-gate -- --plan-all --json`; `runtime:pi-live-gate -- --plan <gate-id>`; `runtime:pi-live-gate -- --validate-args <gate-id> --strict --json`; `runtime:pi-controlled-live-turn-gate`; `runtime:pi-live-send-message-gate`; `runtime:pi-live-send-media-gate`; `runtime:pi-live-notification-gate`; `runtime:pi-cron-notification-gate`; `runtime:pi-buildroom-handoff-gate`; `runtime:pi-admin-config-gate`; `runtime:pi-mcp-file-transfer-gate`; `runtime:pi-honcho-local-gate`; `runtime:pi-learning-propose-gate`; `runtime:pi-memory-read-gate`; `docs/runtime-side-effect-gates.md` | L1-L2 complete for the reusable harness: controlled group live turn, message, media, notification, fake cron/notification, temp-only scheduled work, temp-only Buildroom handoff, temp-only admin/config, temp-only MCP/file-transfer, temp-only Honcho local, propose-only learning, read-only memory gates, aggregate dispatcher, registry display metadata, no-run plan catalog, strict argument validation, package-script drift checks, and docs | None for the generic harness. Remaining side-effect work is expansion-specific live evidence for concrete agents/channels, not new hardcoded parity-smoke extraction. |
...
| `memory.search-write-review` | memory_learning | `pi.sessions-memory-learning`; `runtime:pi-memory-read-gate`; `runtime:pi-timur-agent-memory-read-smoke` | L2-L4 | Live memory write paths should be agent-gated; `pi_telegram_lab` exposes operator memory commands but uses propose-only learning. |
...
| `project-manager` | high by audit queue | Multi-root audit passed with coverage gap false and expansion packet recorded in `research/pi-expansion-packets/project-manager.md`. Automated pre-live evidence passed for monitor, external MCP, plugin/context coverage, generic Telegram group-route preflight, generic scheduled-work gate, and propose-only learning posture; allowlisted peer/thread confirmation is closed by explicit account/peer/topic match. The packet now keeps explicit operator go/no-go open so monitor alone cannot close a high-risk group expansion. | Close explicit operator go/no-go and post-expansion monitor before treating it as live-expanded. |
...
4. Run `runtime:pi-expansion-status -- --packets-dir research/pi-expansion-packets ...` to get the machine-readable open/closed next-action list; add `--fail-on-open --allow-external-open` when CI should accept only external operator approval and follow-up monitor evidence as the remaining queue.
...
The expected parity is strong enough to keep Pi as the default AnthroClaw runtime now. The generic replacement harness is in place: Runtime v1 owns the contract, Pi is the default provider path, and side-effect gates are reusable across agents instead of tied to a lab agent. The remaining work is not "does Pi replace the Agent SDK harness?" but "which exact production agents and side-effect surfaces have been accepted under the new harness?"

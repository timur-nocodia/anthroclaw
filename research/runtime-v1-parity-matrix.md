...
| Fleet expansion progress view | Complete | `runtime:pi-expansion-status` combines multi-root audit output with `research/pi-expansion-packets` and reports closed/open/missing packet state plus next actions in human or JSON form. It also classifies open evidence by kind and supports `--fail-on-open --allow-open-kind <kind>` for CI/operator gates that should pass only when all remaining work is explicitly allowed; output includes the evaluated policy, exit code, reason, and concrete policy violations. |
...
| `tools.side-effect-gates` | tools_permissions | `src/runtime/side-effect-gates/registry.ts`; `src/runtime/__tests__/side-effect-gate-registry.test.ts`; `src/runtime/__tests__/side-effect-gate.test.ts`; `src/runtime/__tests__/live-send-message-gate.test.ts`; `src/runtime/__tests__/live-send-media-gate.test.ts`; `src/runtime/__tests__/live-notification-gate.test.ts`; `src/runtime/__tests__/cron-notification-gate.test.ts`; `src/runtime/__tests__/buildroom-handoff-gate.test.ts`; `src/runtime/__tests__/admin-config-gate.test.ts`; `src/runtime/__tests__/mcp-file-transfer-gate.test.ts`; `src/runtime/__tests__/honcho-local-gate.test.ts`; `src/runtime/__tests__/learning-propose-gate.test.ts`; `src/runtime/__tests__/memory-read-gate.test.ts`; `src/cli/__tests__/pi-live-gate.test.ts`; `runtime:pi-live-gate -- --list --json`; `runtime:pi-live-gate -- --describe <gate-id> --json`; `runtime:pi-live-gate -- --plan-all --json`; `runtime:pi-live-gate -- --plan <gate-id>`; `runtime:pi-live-gate -- --validate-args <gate-id> --strict --json`; `runtime:pi-live-send-message-gate`; `runtime:pi-live-send-media-gate`; `runtime:pi-live-notification-gate`; `runtime:pi-cron-notification-gate`; `runtime:pi-buildroom-handoff-gate`; `runtime:pi-admin-config-gate`; `runtime:pi-mcp-file-transfer-gate`; `runtime:pi-honcho-local-gate`; `runtime:pi-learning-propose-gate`; `runtime:pi-memory-read-gate`; `docs/runtime-side-effect-gates.md` | L1-L2 complete for the reusable harness: message, media, notification, fake cron/notification, temp-only Buildroom handoff, temp-only admin/config, temp-only MCP/file-transfer, temp-only Honcho local, propose-only learning, read-only memory gates, aggregate dispatcher, registry display metadata, no-run plan catalog, strict argument validation, package-script drift checks, and docs | None for the generic harness. Remaining side-effect work is expansion-specific live evidence for concrete agents/channels, not new hardcoded parity-smoke extraction. |
...
4. Run `runtime:pi-expansion-status -- --packets-dir research/pi-expansion-packets ...` to get the machine-readable open/closed next-action list; add `--fail-on-open --allow-open-kind operatorApproval --allow-open-kind postExpansionMonitor` when CI should accept only external operator approval and follow-up monitor evidence as the remaining queue.

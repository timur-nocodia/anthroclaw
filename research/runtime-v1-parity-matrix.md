...
| `tools.side-effect-gates` | tools_permissions | `src/runtime/__tests__/side-effect-gate.test.ts`; `src/runtime/__tests__/live-send-message-gate.test.ts`; `runtime:pi-live-send-message-gate`; `docs/runtime-side-effect-gates.md` | L1-L2 for `live-send-message` | `send_media`, notifications, cron, Buildroom, external MCP, and config/admin side-effect gates still need the same generic extraction. |

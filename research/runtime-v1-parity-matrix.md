...
| Fleet expansion progress view | Complete | `runtime:pi-expansion-status` combines multi-root audit output with `research/pi-expansion-packets` and reports closed/open/missing packet state plus next actions in human or JSON form. It also classifies open evidence by kind and supports `--fail-on-open --allow-open-kind <kind>` for CI/operator gates that should pass only when all remaining work is explicitly allowed. |
